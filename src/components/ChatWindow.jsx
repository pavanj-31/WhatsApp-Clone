...
function ChatWindow() {
  ...
  useEffect(() => {
    const getUser = async () => {
      const docRef = doc(firestore, "users", receiverId);
    ...
    const msgUnsubscribe = onSnapshot(doc(firestore, "user-chats", chatId), (doc) => {
  ...
  const sendMsg = async () => {
    ...
    if (msg) {
      ...
      if (msgList?.length === 0) {
        ...
        await setDoc(doc(firestore, "user-chats", chatId), {
          chatId: chatId,
          messages: [
            {
              text: msg,
        ...
        await updateDoc(doc(firestore, "user-chats", chatId), {
          chatId: chatId,
  ...
  const isOnline = secondUser?.lastSeen?.toDate
    ? secondUser.lastSeen.toDate() > new Date(Date.now() - 2 * 60 * 1000)
    : false;

  const formattedLastSeen = secondUser?.lastSeen?.toDate
    ? secondUser.lastSeen.toDate().toLocaleString()
    : "";
      ...
      <div className="bg-background py-2 px-4 flex items-center gap-2 shadow-sm">
        ...
        <div >
          ...
          <p className="text-xs text-neutral-400">
            {isOnline ? "Online" : `last seen at ${formattedLastSeen}`}
          </p>
        </div>
      </div>
